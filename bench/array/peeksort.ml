...
let bench_table ~generator ~lengths ~repeat ()
  ...
  List.map
    (fun length ->
       ...
       let t_ts  = lazy (bench_one ~measure:runtime ~algorithm:Peeksort.peeksort ~inputs ()) in
       let c_std = lazy (bench_one ~measure:comparisons ~algorithm:Array.stable_sort ~inputs ()) in
       let c_ts  = lazy (bench_one ~measure:comparisons ~algorithm:Peeksort.peeksort ~inputs ()) in
...
let headers =
  List.map string_cell [
    "length";
    "stdlib"; "peeksort"; "speedup";
    "lg(n!)"; "stdlib"; "peeksort"; "speedup"
  ]
